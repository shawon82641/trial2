...
def load_custom_range():
    try:
        if os.path.exists("custom_range.json"):
            with open("custom_range.json", "r") as f:
                return json.load(f).get("range")
    except:
        pass
    return None

def save_custom_range(rng):
    with open("custom_range.json", "w") as f:
        json.dump({"range": rng}, f)
...
async def setrange_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(uid):
        ...
        return
    if not context.args:
        await update.message.reply_text("Usage: /setrange 3846336XXX")
        return
    rng = context.args[0].strip()
    save_custom_range(rng)
    await update.message.reply_text(f"✅ Custom range set: {rng}")
...
def main_keyboard(user_id):
    keyboard = [
        ...
        [KeyboardButton("📸 Get Fresh Instagram number", style="primary")],
        ...
        try:
            ...
            if "data" in res and "otps" in res["data"]:
                ...
                for otp in otps:
                    ...
                    otp_id = str(otp.get("nid") or otp.get("otp_id", "")).strip()
                    
                    # Duplicate reward protection:
                    # Same number + same OTP = reward only once
                    # Same number + different OTP = reward again
                    ...
                    if (num in active_numbers and 
                        ...
                        # Referral commission from referred user's earnings
                        try:
                            users_db = load_data(USER_DATA_FILE)
                            referred_user = users_db.get(str(details["uid"]), {})
                            referrer_id = referred_user.get("referred_by")
                            if referrer_id and str(referrer_id) in users_db:
                                refer_percent = float(settings.get("refer_bonus", 10.0))
                                commission = round((otp_reward * refer_percent) / 100.0, 6)
                                if commission > 0:
                                    users_db[str(referrer_id)]["balance"] = round(users_db[str(referrer_id)].get("balance",0.0)+commission,4)
                                    users_db[str(referrer_id)]["referral_earnings"] = round(users_db[str(referrer_id)].get("referral_earnings",0.0)+commission,4)
                                    save_data(users_db, USER_DATA_FILE)
...
async def process_auto_number(update, context, range_text):
    ...
    try:
        ...
        final_text = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"<blockquote>📱 APP: <code>{context.user_data.get('selected_app', 'Instagram Premium Number')}</code></blockquote>\n"
...
async def process_numbers(update, context, range_text, count, edit_message=None):
    ...
    try:
        ...
        app_name = context.user_data.get('selected_app', 'Instagram Premium Number')
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /broadcast command
    if text == "/broadcast" and is_admin(uid):
        context.user_data["broadcast_mode"] = True
        await update.message.reply_text("📢 Send any text/photo/video/voice/audio/document to broadcast.")
        return

    # Broadcast media/text handler
    if context.user_data.get("broadcast_mode") and is_admin(uid):
        ...
        await update.message.reply_text(f"✅ Broadcast sent to {ok} users.")
        return
    ...
    if text == "👥 REFER & EARN":
        ...
        refer_msg = (
            ...
            f"📊 Referral Commission: {refer_bonus}%\n\n"
    ...
    if text == "📸 Get Fresh Instagram number":
        rng = load_custom_range()
        if not rng:
            await update.message.reply_text("⚠️ Admin has not set custom range yet.")
            return
        await request_queue.put({'type': 'process_numbers', 'update': update, 'context': context, 'range_text': rng, 'count': 1})
        return
    ...
    if text == "💸 PENDING WITHDRAWALS" and is_admin(uid):
        ...
        for w in pendings:
            pid = w.get("payment_id")
            keyboard = InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ CANCEL", callback_data=f"admin_reject_{pid}", style="danger"),
                InlineKeyboardButton("✅ CONFIRM", callback_data=f"admin_approve_{pid}", style="success")
            ]])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new_user:
        ...
        if settings.get("join_alert_enabled", True):
            ...
            join_source = "Direct Join"

            if context.args and len(context.args) > 0:
                ref_param = context.args[0]
                if ref_param.isdigit() and ref_param != uid_str and ref_param in existing_data:
                    ref_info = existing_data.get(ref_param, {})
                    ref_name = ref_info.get("full_name") or ref_info.get("username") or "Unknown"
                    join_source = f"Referral ({ref_name} | {ref_param})"
                ...
                try:
                    ...
                    await context.bot.send_message(
                        ...
                        text=(
                            ...
                            f"💰 Commission Rate Registered: <code>{settings.get('refer_bonus',10)}%</code>\n⏳ Commission will be credited when the user earns."
                        ),
                        parse_mode="HTML"
                    )
                except Exception as e:
